...
class REST_CRUD_API {
	...
	protected function createObject($input,$tables,$db) {
		...
		foreach ($input as $key => $value) {
			 $decoded_input[urldecode($key)] = urldecode($value);
		}
		$keys = implode('","',str_split(str_repeat('!', count($decoded_input))));
		$values = implode(',',str_split(str_repeat('?', count($decoded_input))));
		$params = array_merge(array_keys($input),array_values($decoded_input));
...
// For MySQL Server use:
// Bytt ut med dine egne verdier:
